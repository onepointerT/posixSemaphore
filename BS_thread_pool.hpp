...
public:
    ...
    blocks(const T first_index_, const T index_after_last_, const std::size_t num_blocks_) noexcept : first_index(first_index_), index_after_last(index_after_last_), num_blocks(num_blocks_)
    {
        ...
        {
            const std::size_t total_size = static_cast<std::size_t>(this->index_after_last - this->first_index);
            this->num_blocks = (std::min)(this->num_blocks, this->total_size);
            this->block_size = this->total_size / this->num_blocks;
            this->remainder = this->total_size % this->num_blocks;
            if (block_size == 0)
            {
                block_size = 1;
                this->num_blocks = (this->total_size > 1) ? this->total_size : 1;
            }
        }
        else
        {
